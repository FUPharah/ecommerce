...
export const ColourForm: React.FC<ColourFormProps> = ({
  ...
  const title = initialData ? "Edit Colour" : "New Colour";
  const description = initialData ? "Edit this Colour" : "Create a new Colour";
  const toastMessage = initialData ? "Colour updated successfully 🎉" : "Colour created successfully 🎉";
  const action = initialData ? "Save Changes" : "Create Colour";
  const toastError = initialData ? "Something went wrong. 🤦‍♂️" : "Make sure you removed all products using this Colour first. ❗❕";
  const toastCancel = initialData ? "❌ Colour update canceled. ❌ " : "❌ Colour creation canceled. ❌";
